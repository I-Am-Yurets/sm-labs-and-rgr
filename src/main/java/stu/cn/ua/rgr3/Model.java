...
/**
 * Model – модель супермаркету (Варіант 19).
 *
 * Компоненти моделі:
 *  - customer        – прототип покупця (клонується у multiCustomer)
 *  - multiCustomer   – множина покупців (кількість задається у GUI)
 *  - cashier         – прототип касира (клонується у multiCashier)
 *  - multiCashier    – множина касирів (кількість задається у GUI)
...
public class Model {
    ...
    /** Зв'язує компоненти моделі з діаграмами GUI та налаштовує протокол. */
    ...
    // ── Ліниві ініціалізатори компонентів ────────────────────────────────────
    ...
    public MultiActor getMultiCustomer() {
        if (multiCustomer == null) {
            // Кількість потоків покупців = кількість кас × 3 (умовно)
            int streams = Math.max(1, gui.getChooseDataCashiers().getInt() * 3);
            multiCustomer = new MultiActor();
            multiCustomer.setNameForProtocol("Потоки покупців");
            multiCustomer.setOriginal(getCustomer());
            multiCustomer.setNumberOfClones(streams);
        }
