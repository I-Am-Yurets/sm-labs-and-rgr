...
/**
 * Варіант 19 – Моделювання роботи супермаркету (РГР-7).
 * Повна версія: StatisticsManager + ExperimentManager + TransProcessManager.
 */
public class Main extends JFrame {
    ...
    public static void main(String[] args) {
        try { UIManager.setLookAndFeel(new FlatDarculaLaf()); } catch (Exception e) { e.printStackTrace(); }
    ...
    private void initComponents() {
        ...
        transProcessManager      = new TransProcessManager() {
            ...
            public Dispatcher getDispatcher() {
                // Завжди створюємо новий диспетчер, щоб можна було запускати повторно
                try {
                    java.lang.reflect.Field f = TransProcessManager.class.getDeclaredField("dispatcher");
                    f.setAccessible(true);
                    Dispatcher d = (Dispatcher) f.get(this);
                    if (d == null || !d.getThread().isAlive()) {
                        d = new Dispatcher();
                        d.setProtocolFileName("");
                        f.set(this, d);
                    }
                    return d;
                } catch (Exception ex) {
        ...
        setupChooseData(chooseDataMaxCustomersInStore, "Макс. покупців у залі", 20);
        leftSettingModelPanel.add(chooseDataMaxCustomersInStore, "cell 0 6,aligny center,growy 0");

        setupChooseData(chooseDataPurchasesPerCustomer, "Середня кількість покупок", 5);
        ...
        // Transient tab — підключаємо TransProcessManager
        transientPanel.setLayout(new MigLayout("hidemode 3", "[646,grow,fill]", "[grow]rel"));
    ...
    private void setupChooseData(ChooseData cd, String title, int defaultVal) {
        cd.setBackground(new Color(0x3c3f41));
        cd.setTitle(title);
    ...
    private void setupDiagram(Diagram d, String title, Color painterColor) {
        ...
        d.setPanelBackground(new Color(0x575959));
        d.setGridColor(new Color(0x999999));
